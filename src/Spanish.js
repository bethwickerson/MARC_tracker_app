...
import LogoTrackerEs from './LogoTracker_es'
    ...
    <>
      <div className="header">
        <div className="button-container">
          <button className={`download-open ${dropMenu === true ? "show" : "hide"}`} onClick={downloadOption}>Descargar PNG</button>
          <button className={`embed-open ${embedCode === true ? "show" : "hide"}`} onClick={embedOption}>Código de inserción</button>
        </div>
      </div>
      ...
      <div className={`download-option ${dropMenu === true ? "show" : "hide"}`}>
        <button onClick={downloadPNG_Low}>Baja resolucion (2040x2040, 343KB)</button>
        <button onClick={downloadPNG_Med}>Media resolucion (6800x6800, 1.9MB)</button>
        <button onClick={downloadPNG_High}>Alta resolución (13600x13600, 5.6MB)</button>
      </div>
      <div className={`embed-code ${embedCode === true ? "show" : "hide"}`}>
        <div className="center-code">
          <h4>copie / pegue el siguiente código html en su página web:</h4>
          <hr />
          <code>
            ...
            &lt;iframe src=&quot;http://marc-kc.github.io/MARC_tracker_app/#/spanish&quot;
      ...
      <div className={`loading ${loading === true ? "show" : "hide"}`}><h1>cargando...</h1></div>
    </>
  );
}